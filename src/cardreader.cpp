...
void CardReader::init(Local<Object> target) {
  ...
  Nan::SetPrototypeTemplate(tpl, "_connect",
                            Nan::New<FunctionTemplate>(Connect));
  Nan::SetPrototypeTemplate(tpl, "_reconnect",
                            Nan::New<FunctionTemplate>(Reconnect));
  Nan::SetPrototypeTemplate(tpl, "_disconnect",
                            Nan::New<FunctionTemplate>(Disconnect));
  Nan::SetPrototypeTemplate(tpl, "_transmit",
                            Nan::New<FunctionTemplate>(Transmit));
  Nan::SetPrototypeTemplate(tpl, "_control",
                            Nan::New<FunctionTemplate>(Control));
  Nan::SetPrototypeTemplate(tpl, "_get_status",
                            Nan::New<FunctionTemplate>(Status));
  Nan::SetPrototypeTemplate(tpl, "close", Nan::New<FunctionTemplate>(Close));

  // PCSCLite constants
  // Share Mode
  Nan::SetPrototypeTemplate(tpl, "SCARD_SHARE_SHARED",
                            Nan::New(SCARD_SHARE_SHARED));
  Nan::SetPrototypeTemplate(tpl, "SCARD_SHARE_EXCLUSIVE",
                            Nan::New(SCARD_SHARE_EXCLUSIVE));
  Nan::SetPrototypeTemplate(tpl, "SCARD_SHARE_DIRECT",
                            Nan::New(SCARD_SHARE_DIRECT));

  // Protocol
  Nan::SetPrototypeTemplate(tpl, "SCARD_PROTOCOL_T0",
                            Nan::New(SCARD_PROTOCOL_T0));
  Nan::SetPrototypeTemplate(tpl, "SCARD_PROTOCOL_T1",
                            Nan::New(SCARD_PROTOCOL_T1));
  Nan::SetPrototypeTemplate(tpl, "SCARD_PROTOCOL_RAW",
                            Nan::New(SCARD_PROTOCOL_RAW));

  //  State
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_UNAWARE",
                            Nan::New(SCARD_STATE_UNAWARE));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_IGNORE",
                            Nan::New(SCARD_STATE_IGNORE));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_CHANGED",
                            Nan::New(SCARD_STATE_CHANGED));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_UNKNOWN",
                            Nan::New(SCARD_STATE_UNKNOWN));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_UNAVAILABLE",
                            Nan::New(SCARD_STATE_UNAVAILABLE));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_EMPTY",
                            Nan::New(SCARD_STATE_EMPTY));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_PRESENT",
                            Nan::New(SCARD_STATE_PRESENT));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_ATRMATCH",
                            Nan::New(SCARD_STATE_ATRMATCH));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_EXCLUSIVE",
                            Nan::New(SCARD_STATE_EXCLUSIVE));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_INUSE",
                            Nan::New(SCARD_STATE_INUSE));
  Nan::SetPrototypeTemplate(tpl, "SCARD_STATE_MUTE",
                            Nan::New(SCARD_STATE_MUTE));

  // Disconnect disposition
  Nan::SetPrototypeTemplate(tpl, "SCARD_LEAVE_CARD",
                            Nan::New(SCARD_LEAVE_CARD));
  Nan::SetPrototypeTemplate(tpl, "SCARD_RESET_CARD",
                            Nan::New(SCARD_RESET_CARD));
  Nan::SetPrototypeTemplate(tpl, "SCARD_UNPOWER_CARD",
                            Nan::New(SCARD_UNPOWER_CARD));
  Nan::SetPrototypeTemplate(tpl, "SCARD_EJECT_CARD",
                            Nan::New(SCARD_EJECT_CARD));
...
NAN_METHOD(CardReader::New) {
  ...
  Nan::Set(obj->handle(), Nan::New(name_symbol),
...
NAN_METHOD(CardReader::GetStatusChange) {
  ...
  uv_async_init(uv_default_loop(), &async_baton->async,
                (uv_async_cb)HandleReaderStatusChange);
  int ret =
      uv_thread_create(&obj->m_status_thread, HandlerFunction, async_baton);
  assert(ret == 0);
}
...
NAN_METHOD(CardReader::Reconnect) {
  ...
  // after the threadpool function completed.
  int status =
      uv_queue_work(uv_default_loop(), &baton->request, DoReconnect,
...
NAN_METHOD(CardReader::Disconnect) {
  ...
  // after the threadpool function completed.
  int status =
      uv_queue_work(uv_default_loop(), &baton->request, DoDisconnect,
...
NAN_METHOD(CardReader::Status) {
  ...
  if (!info[1]->IsUint32()) {
    ...
    return Nan::ThrowError(
        "Second argument must be an integer for reader name length");
  }
...
void CardReader::HandleReaderStatusChange(uv_async_t *handle, int status) {
  ...
  if (reader->m_state == 1) {
    // Swallow events : Listening thread was cancelled by user.
             ...
             (ar->result ==
              (LONG)SCARD_E_UNKNOWN_READER)) { // Card reader was unplugged,
                                               // it's not an error
    if (ar->status != 0) {
      ...
      Local<Value> argv[argc] = {
          ...
          Nan::CopyBuffer(reinterpret_cast<char *>(ar->atr), ar->atrlen)
              .ToLocalChecked()};
    ...
    Local<Value> err =
        Nan::Error(error_msg("SCardGetStatusChange", ar->result).c_str());
...
void CardReader::HandlerFunction(void *arg) {
  ...
  while (!reader->m_state) {
    ...
    uv_mutex_lock(&reader->m_mutex);
    if (reader->m_state == 1) {
      // Exit requested by user. Notify close method about SCardStatusChange was
      // interrupted.
    ...
    memcpy(async_baton->async_result->atr, card_reader_state.rgbAtr,
           card_reader_state.cbAtr);
...
void CardReader::DoConnect(uv_work_t *req) {
  ...
  if (result == SCARD_S_SUCCESS) {
    result =
        SCardConnect(obj->m_card_context, obj->m_name.c_str(), ci->share_mode,
...
void CardReader::AfterConnect(uv_work_t *req, int status) {
  ...
  if (cr->result) {
    Local<Value> err =
        Nan::Error(error_msg("SCardConnect", cr->result).c_str());
    // Prepare the parameters for the callback function.
...
void CardReader::DoReconnect(uv_work_t *req) {
  ...
  if (result == SCARD_S_SUCCESS) {
    result =
        SCardReconnect(obj->m_card_handle, ri->share_mode, ri->pref_protocol,
...
void CardReader::AfterReconnect(uv_work_t *req, int status) {
  ...
  if (rr->result) {
    Local<Value> err =
        Nan::Error(error_msg("SCardReconnect", rr->result).c_str());
    // Prepare the parameters for the callback function.
    const unsigned argc = 1;
    Local<Value> argv[argc] = {err};
    Nan::Call(Nan::Callback(Nan::New(baton->callback)), argc, argv);
...
void CardReader::AfterDisconnect(uv_work_t *req, int status) {
  ...
  if (*result) {
    Local<Value> err =
        Nan::Error(error_msg("SCardDisconnect", *result).c_str());
...
void CardReader::DoTransmit(uv_work_t *req) {
  ...
  uv_mutex_lock(&obj->m_mutex);
  /* Connected? */
  // Under windows, SCARD_IO_REQUEST param must be NULL. Else error
  // RPC_X_BAD_STUB_DATA / 0x06F7 on each call.
  if (obj->m_card_handle) {
    SCARD_IO_REQUEST send_pci = {ti->card_protocol, sizeof(SCARD_IO_REQUEST)};
    result = SCardTransmit(obj->m_card_handle, &send_pci, ti->in_data,
                           ti->in_len, NULL, tr->data, &tr->len);
  }
...
void CardReader::AfterTransmit(uv_work_t *req, int status) {
  ...
  if (tr->result) {
    Local<Value> err =
        Nan::Error(error_msg("SCardTransmit", tr->result).c_str());
    ...
    Local<Value> argv[argc] = {
        Nan::Null(),
        Nan::CopyBuffer(reinterpret_cast<char *>(tr->data), tr->len)
            .ToLocalChecked()};
...
void CardReader::DoControl(uv_work_t *req) {
  ...
  /* Connected? */
  if (obj->m_card_handle) {
    result = SCardControl(obj->m_card_handle, ci->control_code, ci->in_data,
                          ci->in_len, ci->out_data, ci->out_len, &cr->len);
  }
...
void CardReader::AfterControl(uv_work_t *req, int status) {
  ...
  if (cr->result) {
    Local<Value> err =
        Nan::Error(error_msg("SCardControl", cr->result).c_str());
...
void CardReader::DoStatus(uv_work_t *req) {
  ...
  if (obj->m_card_handle) {
    result =
        SCardStatus(obj->m_card_handle, ci->reader_name, &ci->reader_name_len,
                    &cr->state, &cr->protocol, ci->attr, &ci->attr_len);
  }
...
void CardReader::AfterStatus(uv_work_t *req, int status) {
  ...
  if (cr->result) {
    ...
    Local<Value> argv[argc] = {
        Nan::Null(), Nan::New<Number>(cr->reader_name_len),

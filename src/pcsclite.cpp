...
PCSCLite::PCSCLite()
    : m_card_context(0), m_card_reader_state(), m_status_thread(0), m_state(0) {
...
void PCSCLite::HandleReaderStatusChange(uv_async_t *handle, int status) {
  ...
  if (async_baton->pcsclite->m_state == 1) {
    ...
    Local<Value> argv[argc] = {
        ...
        Nan::CopyBuffer(ar->readers_name, ar->readers_name_length)
            .ToLocalChecked()};
  ...
  while (!pcsclite->m_state) {
    ...
    if (result != SCARD_S_SUCCESS) {
      async_baton->async_result->err_msg =
          error_msg("SCardListReaders", result);
    }
...
LONG PCSCLite::get_card_readers(PCSCLite *pcsclite, AsyncResult *async_result) {
  ...
  result = SCardListReaders(pcsclite->m_card_context, NULL,
                            (LPTSTR)&readers_name, &readers_name_length);
#else
  /* Find out ReaderNameLength */
  result = SCardListReaders(pcsclite->m_card_context, NULL, NULL,
                            &readers_name_length);
  if (result != SCARD_S_SUCCESS) {
    return result;
  }
  ...
  result = SCardListReaders(pcsclite->m_card_context, NULL, readers_name,
                            &readers_name_length);
#endif
